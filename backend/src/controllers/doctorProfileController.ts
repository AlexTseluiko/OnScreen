...
export const createOrUpdateDoctorProfile = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role !== UserRole.DOCTOR && req.user.role !== UserRole.ADMIN) {
      return res.status(400).json({ message: 'Профиль доктора может быть создан только для пользователей с ролью доктора' });
    }
    ...
    const populatedProfile = await DoctorProfile.findOne({ userId })
      .populate('userId', 'firstName lastName email avatar');
...
export const verifyDoctorProfile = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user.role !== UserRole.ADMIN) {
      return res.status(403).json({ message: 'Только администраторы могут верифицировать профили докторов' });
    }
    ...
    res.status(200).json({ message: `Профиль доктора успешно ${verified ? 'верифицирован' : 'деверифицирован'}` });
  } catch (error) {
