...
const getSocketManager = () => {
  if (!socketManager) {
    // Здесь нужно получить экземпляр сервера
    const server = global.server as Server | undefined;
    if (server) {
...
router.post('/request-doctor-role', auth, async (req: AuthRequest, res) => {
  try {
    ...
    for (const admin of admins) {
      ...
      if (socketManager) {
        socketManager.sendNotification(admin._id.toString(), {
          type: NotificationType.SYSTEM,
          title: 'Новый запрос на роль доктора',
          message: `Пользователь ${user.firstName} ${user.lastName} запросил роль доктора. Требуется рассмотрение.`,
        });
...
router.get('/doctor-request-status', auth, async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.id;
    ...
    const request = await DoctorRequest.findOne({ user: userId }).sort({ createdAt: -1 });
