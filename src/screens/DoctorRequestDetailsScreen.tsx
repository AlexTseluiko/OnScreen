...
export const DoctorRequestDetailsScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const loadRequestDetails = useCallback(async () => {
    try {
      ...
      if (response.data.success && response.data.data.request) {
        setRequest(response.data.data.request);
      } else {
        setError(response.data.message || 'Не удалось загрузить детали запроса');
      }
    } catch (error) {
  ...
  const handleApproveRequest = async () => {
    try {
      ...
      if (response.data.success && response.data.data.success) {
        ...
        setError(response.data.message || 'Произошла ошибка при одобрении запроса');
      }
    } catch (error) {
  ...
  const handleRejectRequest = async () => {
    try {
      ...
      if (response.data.success && response.data.data.success) {
        ...
        setError(response.data.message || 'Произошла ошибка при отклонении запроса');
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <View style={[styles.card, { backgroundColor: theme.colors.background }]}>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: theme.colors.text.primary }]}>Документы</Text>
          <Text style={[styles.text, { color: theme.colors.text.primary }]}>
